...
SUBROUTINE init_rng(myid)
    ...
    INTEGER :: clock, seq_myid
    INTEGER(K4B) :: h1, h2

    ! 1. 初始化分配 (假设只需要标量生成，如果需要向量，参数改为需要的长度)
    call ran_init(1) 

    ! 2. 获取系统时钟
    CALL SYSTEM_CLOCK(count=clock)

    ! 3. 混合 myid 和 clock 生成唯一的 sequence
    ! 避免使用 sleep，通过数学运算保证差异
    ! 使用在大质数上的线性运算来打散 bits
    h1 = int(clock, K4B)
    h2 = int(myid, K4B)
    
    ! 简单的哈希组合，确保即使 clock 相同，myid 不同 resulting seq 也不同
    seq_myid = ieor(h1 * 1664525_K4B + 1013904223_K4B, &
                    h2 * 214013_K4B + 2531011_K4B)
...
SUBROUTINE init_rng_by_seed(iseed)
    ...
    ! 分开调用，避免 if-else 逻辑陷阱
    CALL ran_seed(size=n) 
    
    ALLOCATE (seed(n))
    seed = iseed 
    ! 注意：简单的 seed = iseed 会让数组所有元素都一样

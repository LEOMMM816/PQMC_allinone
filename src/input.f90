...
contains
  ...
  subroutine general_paramter_init()
    ...
    namelist /PHONON_ARGUMENTS/ n_phonon_field, D, M, omega, ep_parameter,end_field,n_local_update,jump_distance
    namelist /GLOBAL_UPDATE_PARAM/ global_update_loop,global_update_distance,n_global_update

    ! 1. 打印“我现在在哪？” (调用 Shell 命令 pwd)
    !call execute_command_line("echo 'DEBUG: Current PWD is: ' $(pwd)")

    ! 2. 打印“我看到的文件列表” (确认 model 目录是否存在)
    !call execute_command_line("ls -R")

    ! 4. 使用 Fortran 内部查询确认文件存在性
    inquire(file=trim(nml_file), exist=file_exists)
    if (file_exists) then
      !print *, "DEBUG: Fortran found the file!"
    else
      print *, "DEBUG: Fortran says file MISSING!"
    end if
    open(10, file=nml_file, status='old',position = 'rewind')
    !print *, "DEBUG: Successfully opened the file."
    read(10, nml=GENERAL_ARGUMENTS)
    !print *, "DEBUG: Successfully read GENERAL_ARGUMENTS."
    rewind(10)
    ...
    rewind(10)
    read(10, nml=GLOBAL_UPDATE_PARAM)
    !print *, "DEBUG: Successfully read global_update."
    close(10)

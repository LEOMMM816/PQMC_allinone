...
! run time
  ...
  ! lattice initialize
  !print *, 'initializing general parameters...'
...
! ////////////////////////////////////////////////////////////////////////////////////////
  ...
  subroutine init_boson_field()
    ...
    if (.not. import_ph_field) then ! set up ph field from new
      ...
      read(nml_unit, nml=bf_initial, iostat=ios)
      if (ios /= 0) then
      ...
      do l = 1, ntime
        ...
        do i_cell = 1, Lat%N_cell
          phase_factor = exp(cmplx(0d0,sum(phase_k_inter_cell * p_cells(i_cell)%rpos * PI),kind=dp))
          do i_bf = 1, bf_sets
  ...
  subroutine my_mpi_init()
    ...
    MPI_nblock = 0
    call get_environment_variable("nblock", value=ci1, status=ios)
    !MPI_nblock = CPPBLOCK
    if (ios /= 0) then
      if (myid == 0) then
        print *, "ERROR: 环境变量 nblock 未设置"
        print *, "ios =", ios
      end if
      call MPI_Abort(MPI_COMM_WORLD, 1, ierr)
    else
      ! ci1 contains the string value of nblock
      read(ci1,*) MPI_nblock
      if (myid == 0) then
        print *, "nblock =", MPI_nblock
      end if
    end if
    MPI_one_block = numprocs/MPI_nblock
    ...
    ! find the nmlfile from model_name
    write(block_str, '(1i4)') mpi_block_id
    nml_file = trim(adjustl(model_name))//'_b'//trim(adjustl(block_str))//'.nml'
    !print*,trim(nml_file)
    !print*,'model/'//trim(adjustl(nml_file))
    nml_file = 'model/'//trim(adjustl(nml_file))
    if (myid == 0) then

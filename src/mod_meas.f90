module dqmc_measurements
  ...
  contains
    ...
    procedure :: compute_one_bin_mean    => ms_compute_one_bin_mean
    ! ---- output ----
    procedure :: output_all_data => ms_output_all_data
    procedure :: output_bin_data => ms_output_bin_data
    procedure :: output_bin_count => ms_output_bin_count
...
subroutine spin_current_matrix(pc1,pc1_nb,pc2,pc2_nb, mat)
  implicit none
  type(cell_type), pointer,intent(in) :: pc1, pc1_nb, pc2, pc2_nb
  complex(dp), intent(out) :: mat(4, 4)
  integer :: i,j
  ! compute the spin current matrix element <ci1* cj2 , ci2* cj1> for two bonds (pc1, pc1_nb) and (pc2, pc2_nb)
  integer :: left_sites(2,2), right_sites(2,2)
  left_sites(:,1) = [pc1%sites(1),pc1_nb%sites(1)]
  left_sites(:,2) = [pc1_nb%sites(1),pc1%sites(1)]
  right_sites(:,1) =[pc2%sites(1),pc2_nb%sites(1)]
  right_sites(:,2) =[pc2_nb%sites(1),pc2%sites(1)]
  mat = 0.0d0
  ! first the spin-up part
    do i = 1,2
      do j = 1,2
    ...
    ! spin-down part is the conjugate of spin-up part
    mat(3:4,3:4) = conjg(mat(1:2,1:2))  
    
  ! the mixture part
    do i = 1,2
      do j = 1,2
        ! hatree part only, no fock part since different spin species
        mat(i,j+2) = mat(i,j+2) + (g_h(left_sites(i,2),left_sites(i,1)) * conjg(g_h(right_sites(j,2),right_sites(j,1)))) 
        mat(i+2,j) = mat(i+2,j) + conjg(g_h(left_sites(i,2),left_sites(i,1))) * g_h(right_sites(j,2),right_sites(j,1))  
      end do
    end do
  ...
  subroutine cal_wick_contraction(mat, n_g, left_sites, right_sites, hatree_only)
    ...
    complex(dp), intent(inout) :: mat(n_g,n_g)
...
!=========================main measurement subroutine========================!
  SUBROUTINE ms_take_measurement(this, time)
    ...
    integer ::   i,j,c1,c2,s1,bf1_x,bf1_y, s2,bf2_x,bf2_y
    complex(dp) :: t1x,t1y,t2x,t2y
    complex(dp) :: obs_temp
    type(cell_type), pointer :: pc1,pc1_x,pc1_y, pc2, pc2_x,pc2_y
    complex(dp), allocatable :: vec_l(:), vec_r(:)
    complex(dp) :: spinJz_mat(4,4),spinM_mat(4,4)
    complex(dp) :: phy_spinJxy_mat(8,8)
    logical :: cross_boundary_1(2) = (/.false., .false./), cross_boundary_2(2) = (/.false., .false./)
    complex(dp) :: Jx,Jy,Jxy,Jcx,Jcy,Jcxy
    La = Lat%dlength(1)
    Lb = Lat%dlength(2)
    N_cell = Lat%N_cell
    head = this%cur_meas
    ...
    !! preparation
      ...
      t1x = hop + (0d0,1d0) * ep_parameter * boson_field(bf1_x,time)
      t1y = hop + (0d0,1d0) * ep_parameter * boson_field(bf1_y,time)
    ...
    !! corfs are measured between two unit cells, pc2 is introduced
      ...
      !! preparation for correlation functions
        ...
        ! cross_boundary_i(x) contains if site_i's x-direction neighbor crosses the boundary
        ! cross_boundary_i(y) contains if site_i's y-direction neighbor crosses the boundary
        s2 = pc2%sites(1)
        bf2_x = pc2%bf_list(1)
        bf2_y = pc2%bf_list(2)
        t2x = hop + (0d0,1d0) * ep_parameter * boson_field(bf2_x,time)
        t2y = hop + (0d0,1d0) * ep_parameter * boson_field(bf2_y,time)
      ...
      !! corf: bfxy-bfxy
        ...
        call this%record_field_entry(handle, ind, obs_temp)
      !! corf1: x-bond's spin-z current correlation
        handle = this%get_handle('Jx_Jx')
        obs_temp = 0d0
        vec_l = hop * (0,1d0) * [-1d0,1d0,1d0,-1d0] ! spin-z current vector
          vec_r = vec_l
        call spin_current_matrix(pc1,pc1_x,pc2,pc2_x,spinJz_mat) ! x-bond's spinJ mat
        call add_boundary_phase(vec_l,  cross_boundary_1, 1)
        call add_boundary_phase(vec_r,  cross_boundary_2, 1)
        obs_temp = obs_temp + sum(vec_l * matmul(spinJz_mat, vec_r))/N_cell
        Jx = obs_temp
        call this%record_field_entry(handle, ind, obs_temp)
      
      !! corf1' : x-bond's physical spin-z current correlation
        handle = this%get_handle('Jcx_Jcx')
        obs_temp = 0d0
        vec_l = (0,1d0)  * [-1d0*(t1x),1d0*conjg(t1x),1d0*conjg(t1x),-1d0 * (t1x)] ! s1x physical spin-z current vector
        vec_r = (0,1d0)  * [-1d0*(t2x),1d0*conjg(t2x),1d0*conjg(t2x),-1d0 * (t2x)] ! s2x physical spin-z current vector
        ! spinJz_mat now contains x-bond's spinJ
        call add_boundary_phase(vec_l,  cross_boundary_1, 1)
        call add_boundary_phase(vec_r,  cross_boundary_2, 1)
        obs_temp = obs_temp + sum(vec_l * matmul(spinJz_mat, vec_r))/N_cell
        Jcx = obs_temp
      ...
      !! corf2: y-bond's spin-z current correlation
        handle = this%get_handle('Jy_Jy')
        obs_temp = 0d0
        vec_l = hop * (0,1d0) * [-1d0,1d0,1d0,-1d0] ! spin-z current vector
        vec_r = vec_l
        call add_boundary_phase(vec_l,  cross_boundary_1, 2)
        call add_boundary_phase(vec_r,  cross_boundary_2, 2)
        call spin_current_matrix(pc1,pc1_y,pc2,pc2_y,spinJz_mat) ! y-bond's spinJ mat
        obs_temp = obs_temp + sum(vec_l * matmul(spinJz_mat, vec_r))/N_cell
        Jy = obs_temp
        call this%record_field_entry(handle, ind, obs_temp)
      !! corf2' : y-bond's physical spin-z current correlation
        handle = this%get_handle('Jcy_Jcy')
        obs_temp = 0d0
        vec_l = (0,1d0)  * [-1d0*t1y,1d0*conjg(t1y),1d0*conjg(t1y),-1d0 * (t1y)] ! s1y physical spin-z current vector
        vec_r = (0,1d0)  * [-1d0*t2y,1d0*conjg(t2y),1d0*conjg(t2y),-1d0 * (t2y)] ! s2y physical spin-z current vector
        ! spinJz_mat now contains y-bond's spinJ
        call add_boundary_phase(vec_l,  cross_boundary_1, 2)
        call add_boundary_phase(vec_r,  cross_boundary_2, 2)
        obs_temp = obs_temp + sum(vec_l * matmul(spinJz_mat, vec_r))/N_cell
        Jcy = obs_temp
      ...
      !! corf3: y-bond's spin-x current correlation
        handle = this%get_handle('Jx_Jy')
        obs_temp = 0d0
        vec_l = hop * (0,1d0) * [-1d0,1d0,1d0,-1d0] ! spin-z current vector
        vec_r = vec_l
        call add_boundary_phase(vec_l,  cross_boundary_1, 1)
        call add_boundary_phase(vec_r,  cross_boundary_2, 2)
        call spin_current_matrix(pc1,pc1_x,pc2,pc2_y,spinJz_mat) 
        obs_temp = obs_temp + sum(vec_l * matmul(spinJz_mat, vec_r))/N_cell
        ! symmetrized 
        vec_l = hop * (0,1d0) * [-1d0,1d0,1d0,-1d0] ! spin-z current vector
        vec_r = vec_l
        call add_boundary_phase(vec_l,  cross_boundary_1, 2)
        call add_boundary_phase(vec_r,  cross_boundary_2, 1)
        call spin_current_matrix(pc1,pc1_y,pc2,pc2_x,spinJz_mat)
        obs_temp = obs_temp + sum(vec_l * matmul(spinJz_mat, vec_r))/N_cell
        ! obs_temp = obs_temp * 0.5d0
        Jxy = obs_temp
        call this%record_field_entry(handle, ind, obs_temp)

      !! corf3' : x-y bond's physical spin-z current correlation
        handle = this%get_handle('Jcxy_Jcxy')
        obs_temp = 0d0
        vec_l = (0,1d0)  * [-1d0*t1x,1d0*conjg(t1x),1d0*conjg(t1x),-1d0 * (t1x)] ! s1x physical spin-z current vector
        vec_r = (0,1d0)  * [-1d0*t2y,1d0*conjg(t2y),1d0*conjg(t2y),-1d0 * (t2y)] ! s2y physical spin-z current vector
        call add_boundary_phase(vec_l,  cross_boundary_1, 1)
        ...
        ! corf4: J-_J-
        handle = this%get_handle('J-_J-')
        obs_temp = 0d0
        obs_temp = obs_temp + Jx + Jy - Jxy
        call this%record_field_entry(handle, ind, obs_temp)

        !! corf4': Jc-_Jc-
        handle = this%get_handle('Jc-_Jc-')
        obs_temp = 0d0
        obs_temp = obs_temp + Jcx + Jcy - Jcxy
        call this%record_field_entry(handle, ind, obs_temp)
...
!! ---------------------------spin_momentum_correlation_begin----------------------------!!
      !! corf: den-den
        ...
        obs_temp = 0d0
        obs_temp = obs_temp + (g_h(s1,s1) + conjg(g_h(s1,s1)) -1d0) * &
        & (g_h(s2,s2) + conjg(g_h(s2,s2)) -1d0)/N_cell ! hatree part
        obs_temp = obs_temp + (g_h(s2,s1) * g(s1,s2) +  conjg(g_h(s2,s1)*g(s1,s2)))/N_cell ! fock part
        call this%record_field_entry(handle, ind, obs_temp)

      !! corf: SC-SC : <(c1up^+ c1down^+ + c1down c1up) (c2up^+ c2down^+ + c2down c2up)>
        handle = this%get_handle('SC_SC')
        obs_temp = 0d0
        obs_temp = obs_temp + g_h(s2,s1) * conjg(g_h(s2,s1))/N_cell
        obs_temp = obs_temp + g(s1,s2) * conjg(g(s1,s2))/N_cell
        obs_temp = obs_temp * 0.5d0 ! symmetrize
        call this%record_field_entry(handle, ind, obs_temp)

      !! corf: S^z_S^z: <(c1up^+ c1up - c1down^+ c1down) (c2up^+ c2up - c2down^+ c2down)>
        handle = this%get_handle('Sz_Sz')
        obs_temp = 0d0
        obs_temp = obs_temp + (g_h(s1,s1) - conjg(g_h(s1,s1))) * (g_h(s2,s2) - conjg(g_h(s2,s2)))/N_cell
        obs_temp = obs_temp + (g_h(s2,s1) * g(s1,s2) +  conjg(g_h(s2,s1)*g(s1,s2)))/N_cell
        call this%record_field_entry(handle, ind, obs_temp)
      
      !! corf: S^x_S^x = S^y_S^y
        handle = this%get_handle('Sx_Sx')
        obs_temp = 0d0
        obs_temp = obs_temp + g_h(s2,s1) * conjg(g(s1,s2)) + conjg(g_h(s2,s1)) * g(s1,s2)
        call this%record_field_entry(handle, ind, obs_temp)

      !! corf: S^x_S^y
        handle = this%get_handle('Sx_Sy')
        obs_temp = 0d0
        obs_temp = obs_temp + g_h(s2,s1) * conjg(g(s1,s2)) - conjg(g_h(s2,s1)) * g(s1,s2)
        ! symmetrized
        obs_temp = obs_temp + g_h(s1,s2) * conjg(g(s2,s1)) - conjg(g_h(s1,s2)) * g(s2,s1)
        call this%record_field_entry(handle, ind, obs_temp)
  ...
  subroutine ms_k_space_obs(this)
    ...
    do i_obs = 1, this%nobs
      ...
      if (this%kinds(i_obs) == KIND_FIELD) then
        ...
        k_data = k_data/real(lat%N_cell,dp)
        ! record k-space data
        call this%record_field(this%get_handle(trim(this%names(i_obs))//'_k'), k_data)
      end if
    end do
...
!======================================================================!
  ...
  subroutine ms_init_from_namelist(this, filename)
    ...
    if (ios/=0) then 
      write(*,*) "Error opening observable namelist file: "//trim(filename)
      error stop "invilid observable namelist file"
    end if
    do
      name = ''; obskind = ''
      ...
      case default
        write(*,*) "Error: unknown observable obskind: "//trim(obskind)//" (name="//trim(name)//")"
        error stop "invilid observable kind"
      end select
    end do
    close(lun)
  ...
  subroutine push_obs(this, name, k)
    ...
    do i=1,this%nobs
      if (trim(this%names(i)) == trim(name)) then 
        write(*,*) "duplicate observable name: "//trim(name)
        error stop "duplicate observable name"
      end if
    end do
...
!======================================================================!
  ...
  subroutine ms_data_analyse(this)
    ...
    call this%output_all_data()
    
  end subroutine ms_data_analyse

  subroutine ms_compute_one_bin_mean(this, bin)
    ! 计算所有观测量在指定 bin 的均值（沿第 2 维求平均）
    ! 返回: means(total_width)
    class(MeasurementSystem), intent(inout)  :: this
    integer, intent(in) :: bin
    integer :: i_m
    if (bin<1 .or. bin>this%nbins) error stop "compute_one_bin_mean: bin out of range"
    this%mean(:, bin) = 0.0_dp
    do i_m = 1, this%nmeas
      this%mean(:, bin) = this%mean(:, bin) + real(this%data(:, i_m, bin))
    end do
    this%mean(:, bin) = this%mean(:, bin) / real(this%nmeas, dp)

  end subroutine ms_compute_one_bin_mean
  ...
  subroutine ms_output_bin_data(this, bin)
    class(MeasurementSystem), intent(in) :: this
    integer, intent(in) :: bin
    integer :: ios,out_unit
    character(len = 100) :: ci1,ci2,str
    logical :: file_exist
    if (.not.allocated(this%mean)) error stop "output_bin_data: mean not computed"
    if (bin<1 .or. bin>this%nbins) error stop "output_bin_data: bin out of range"
#ifdef MPI
    write (ci1, '(1i4)') myid
    str = 'tempout_core'//trim(adjustl(ci1))//'.bin'
#else
    str = 'tempout_core.bin'
#endif
    str = trim(adjustl(output_addr))//'temp/'//trim(adjustl(str))
    ! check if the file exists, if exists, open the file in append mode
    ! if not, create a new file.
    inquire(file=str, exist=file_exist)
    if (file_exist) then
      open(newunit=out_unit, file=str, status='old', action='write', & 
      & position='append', access='stream', form='unformatted', iostat=ios)
    else
      open(newunit=out_unit, file=str, status='new', action='write',  access='stream', form='unformatted',iostat=ios)
    end if
    if ( ios /= 0) then 
       write(*,*) "output_bin_data: cannot open file: "//trim(str)
        error stop "file open error"
    end if

    ! output the mean data for the specified bin in stream format
    write (unit=out_unit,iostat = ios) this%mean(:, bin)
    close(out_unit)
    call this%output_bin_count(bin)
    end subroutine ms_output_bin_data

    subroutine ms_output_bin_count(this,bin)
    class(MeasurementSystem), intent(in) :: this
    integer, intent(in) :: bin
    integer :: ios,out_unit
    character(len = 100) :: ci1,str
    logical :: file_exist
    ! only called in after each bin is finished to record how many bin mean has been output to temp file
    str = trim(adjustl(output_addr))//'temp/tempout_core_count.dat'
    ! check if the file exists, if exists, open the file in append mode
    ! if not, create a new file.
    inquire(file=str, exist=file_exist)
    if (file_exist) then
      ...
      open(newunit=out_unit, file=str, status='new', action='write', iostat=ios)
    end if
    if ( ios /= 0) then 
       write(*,*) "output_bin_count: cannot open file: "//trim(str)
        error stop "file open error"
    end if  
    write(out_unit,'(A,1i4,A,1i4)') "C", myid, "   |    B", bin
    close(out_unit)
  end subroutine ms_output_bin_count
  subroutine ms_output_all_data(this)

    class(MeasurementSystem), intent(in) :: this
    integer :: ios,out_unit
    character(len = 100) :: ci1,ci2,str

    if (.not.allocated(this%mean)) error stop "output_all_data: mean not computed"
#ifdef MPI
    write (ci1, '(1i4)') myid
    str = 'out_core'//trim(adjustl(ci1))//'.bin'
#else
    str = 'out_core.bin'
#endif
    str = trim(adjustl(output_addr))//'data/'//trim(adjustl(str))
    write(ci2, '(1i4)') this%nbins
    open(newunit=out_unit, file=str, status='replace', access='stream', form='unformatted',iostat=ios)
    if ( ios /= 0) then 
       write(*,*) "output_all_data: cannot open file: "//trim(str)
        error stop "file open error"
    end if
    ...
    write (unit=out_unit,iostat = ios) this%mean
    close(out_unit)
  end subroutine ms_output_all_data
  ...
  subroutine ms_output_reading_guide(this)
    ...
    if (ios /= 0) then 
       write(*,*) "output_reading_guide: cannot open file: "//trim(str)
       error stop "file open error"
    end if
    write(lun, *) '&basics'
    ...
    write(lun, *) 'size_of_dp=', storage_size(x) / 8
#ifdef MPI

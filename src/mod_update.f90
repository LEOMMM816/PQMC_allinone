  ...
  subroutine update_phonon_field(ppf, time)
    ...
    do i_pla = 1 , ppf%n_plaquette
      ...
      if (update_accept) then
        ...
        call gemm(gh_part_nd_temp, gh_part_nd, R_mat_sub, Ns, pfdim, pfdim, (1d0,0d0), (0d0,0d0))
        !DO j = 1, Ns
        !  DO k = 1, pfdim
...
!======================================================================!
  ...
  subroutine generate_newfield_global(update_type)
    ...
    real(dp) :: rot_angle, rot_mat(2,2), rot_tempin(2,ntime),rot_tempout(2,ntime)
    old_field = boson_field
    ! print*,'Performing global update of type:', trim(adjustl(update_type))
    select case (trim(adjustl(update_type)))
    case ("exchange")
    ...
    case ("rotate")
      ...
      ! apply the rotation to the boson field
      do i_cell = 1,Lat%N_cell
        ! boson_field(p_cells(i_cell)%bf_list, :) = matmul(rot_mat, boson_field(p_cells(i_cell)%bf_list, :))
        !call gemm(boson_field(p_cells(i_cell)%bf_list, :), rot_mat, boson_field(p_cells(i_cell)%bf_list, :), &
        !& bf_sets,  bf_sets, ntime, 1d0, 0d0)
        rot_tempin = boson_field(p_cells(i_cell)%bf_list, :)
...
!======================================================================!
  ...
  subroutine cal_prob(w, prob,sign_err)
    ...
    weight = w
    weight = cmplx(real(weight),mod(aimag(weight),2 * pi),kind=dpc) ! make sure weight is in the principal branch
